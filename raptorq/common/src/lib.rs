...
pub fn parse_packet(bytes: &[u8], expected_pid: u16) -> Result<ParsedFrame, ParseError> {
    ...
    match frame_type {
        ...
        FRAME_TYPE_DATA => {
            ...
            let pid_bytes: [u8; 4] = payload
                [FRAME_PAYLOAD_ID_OFFSET..FRAME_PAYLOAD_ID_OFFSET + 4]
                .try_into()
                .unwrap();
            let payload_id = PayloadId::deserialize(&pid_bytes);
            let symbol =
                payload[FRAME_SYMBOL_OFFSET..FRAME_SYMBOL_OFFSET + SYMBOL_SIZE].to_vec();
            Ok(ParsedFrame::Data {
                oti,
                payload_id,
                symbol,
...
mod tests {
    ...
    fn wrong_pid_rejected() {
        ...
        match parse_packet(&buf, 0x101) {
            Err(ParseError::WrongPid { got: 0x100, want: 0x101 }) => {}
