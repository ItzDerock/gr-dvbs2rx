...
use raptorq_ts_common::{
    build_data_packet, build_noop_packet, make_block_oti, ContinuityCounter,
    DEFAULT_BLOCK_SIZE, DEFAULT_PID, DEFAULT_REPAIR_OVERHEAD, SYMBOL_SIZE, TS_PACKET_SIZE,
};
...
fn main() -> Result<()> {
    ...
    let (pkt_tx, pkt_rx) = sync_channel::<EncodingPacket>(args.queue_blocks * (k + repair) as usize);
    ...
    let writer_pid = args.pid;
    let writer_rate = args.rate;
    let writer = thread::Builder::new()
        .name("paced-writer".into())
        .spawn(move || writer_thread(writer_pid, writer_rate, oti, pkt_rx))?;
...
fn reader_thread(block_size: usize, out: SyncSender<Vec<u8>>) -> Result<()> {
    ...
    loop {
        match stdin.read(&mut block[filled..]) {
            Ok(0) => {
                if filled > 0 {
                    warn!(filled, "stdin closed mid-block; padding with zero and flushing");
...
fn writer_thread(
    ...
    loop {
        ...
        stdout.write_all(&buf).context("writing stdout")?;
